...
__global__ void Compute_Slow_Modes(double *SlowMh_d      ,
    ...
    for (int k = 0; k < 3; k++){
        ...
        nflxp_s[iri]+= rscale*(div0 * v_s[ir * 3 + k]  * h_s[ir]  +
                               div1 * v_s[pt1 * 3 + k] * h_s[pt1] +
                               div2 * v_s[pt2 * 3 + k] * h_s[pt2] +
                               div3 * v_s[pt3 * 3 + k] * h_s[pt3] +
...
// Wh
    if (lev == 0){
        ...
        if (NonHydro)    swr = (-diffw_d[id * nv + lev - 1] + advrl + rhol * Gravit) * intt + (-diffw_d[id * nv + lev] + advr + rho * Gravit) * intl;
        else            swr = (-diffw_d[id * nv + lev - 1]         + rhol * Gravit) * intt + (-diffw_d[id * nv + lev]        + rho * Gravit) * intl;
...
// Rho
    nflxr_s[iri] += dwdz;
    SlowRho_d[id * nv + lev] = -nflxr_s[iri] + diffrh_d[id * nv + lev];
...
// pressure
    ...
    Slowpressure_d[id * nv + lev] = (Rd / Cv)*(-nflxp_s[iri] - gtil_d[id * nv + lev] - dwhdz + vgp) + diffpr_d[id * nv + lev];
}
    ...
    if (id < num){
        ...
        for (int lev = 0; lev < nv; lev++){
            ...
            if (lev < nv - 1){
                ...
                for (int i = 1; i < 6; i++)    pressure_p[i] = pressure_d[local_p[i - 1] * nv + lev + 1];
            }
