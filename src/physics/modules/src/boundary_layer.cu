...
bool boundary_layer::initialise_memory(const ESP &              esp,
    ...
    //cudaMalloc((void **)&RiB_d, esp.point_num * sizeof(double));     //holtslag & boville formulation
    cudaMalloc((void **)&RiB_d,
               esp.nvi * esp.point_num * sizeof(double)); //used for Frierson formulation
...
bool boundary_layer::phy_loop(ESP &                  esp,
    ...
    if (bl_type == RAYLEIGHHS) {
        ...
        CalcKM_KH_frierson<<<NBi, NTH>>>(RiB_d,
                                         // L_MO_d,
                                         CD_d,
                                         CH_d,
                                         bl_top_height_d,
...
Momentum_Diff_Impl(double *      Mh_d,
    ...
    if (id < num) {
        ...
        for (lev = 0; lev < bl_top_lev_d[id] + 1; lev++) {
            ...
            if (lev == 0) { //lowest layer, v at lowest boundary = 0, dz0 = Altitude0
                atmp[id * nv + lev] = 0;
                btmp[id * nv + lev] = -(1.0 / (Altitudeh_d[lev + 1] - Altitudeh_d[lev])
                ...
                ctmp[id * nv + lev] = pow(rup, 2) * Rho_int_d[id * (nv + 1) + lev + 1]
                                      * KM_d[id * (nv + 1) + lev + 1]
                                      / ((Altitudeh_d[lev + 1] - Altitudeh_d[lev])
                                         * (Altitude_d[lev + 1] - Altitude_d[lev]));
                ...
                atmp[id * nv + lev] = pow(rlow, 2) * Rho_int_d[id * (nv + 1) + lev]
                                      * KM_d[id * (nv + 1) + lev]
                                      / ((Altitudeh_d[lev + 1] - Altitudeh_d[lev])
                                         * (Altitude_d[lev] - Altitude_d[lev - 1]));
                btmp[id * nv + lev]    = -(1.0 / (Altitudeh_d[lev + 1] - Altitudeh_d[lev])
            ...
            if (lev < bl_top_lev_d[id] + 1) {
                if (!(fabs(btmp[id * nv + lev])
                    ...
                    diagnostics_data[id * nv + lev].data.x = atmp[id * nv + lev];
                    diagnostics_data[id * nv + lev].data.y = btmp[id * nv + lev];
                    diagnostics_data[id * nv + lev].data.z = ctmp[id * nv + lev];
            ...
            for (int k = 0; k < 3; k++) {
                if (lev == bl_top_lev_d[id]) {
                    Mh_d[id * nv * 3 + lev * 3 + k] =
                        dpr_tmp[id * nv * 3 + lev * 3 + k] * Rho_d[id * nv + lev];
                }
                else {
                    Mh_d[id * nv * 3 + lev * 3 + k] =
                        (dpr_tmp[id * nv * 3 + lev * 3 + k]
            ...
            if (lev == -1) { //treat surface in matrix solution (need to offset abcd arrays by +1)
                ...
                btmp[id * nv + lev + 1] =
                    -(Cp / Csurf * Rho_int_d[id * (nv + 1)] * KH_d[id * (nv + 1) + lev + 1]
                          / Altitude_d[lev + 1]
                      + 1.0 / time_step);
                ctmp[id * nv + lev + 1] = Cp / Csurf * Rho_int_d[id * (nv + 1)]
                                          * KH_d[id * (nv + 1) + lev + 1] / Altitude_d[lev + 1];
                cpr_tmp[id * nv + lev + 1] = ctmp[id * nv + lev + 1] / btmp[id * nv + lev + 1];
                ...
                ctmp[id * nv + lev + 1] = Cp / Cv * pow(rup, 2) * Rho_int_d[id * (nv + 1) + lev + 1]
                                          * KH_d[id * (nv + 1) + lev + 1]
                                          / ((Altitudeh_d[lev + 1] - Altitudeh_d[lev])
                                             * (Altitude_d[lev + 1] - Altitude_d[lev]))
                                          * pow(p_int_d[id * (nv + 1) + lev + 1] / P_Ref, kappa);
                btmp[id * nv + lev + 1] = -(atmp[id * nv + lev + 1] + ctmp[id * nv + lev + 1]
                                            + Rho_d[id * nv + lev] / time_step
                ...
                atmp[id * nv + lev + 1] = Cp / Cv * pow(rlow, 2) * Rho_int_d[id * (nv + 1) + lev]
                                          * KH_d[id * (nv + 1) + lev]
                                          / ((Altitudeh_d[lev + 1] - Altitudeh_d[lev])
                                             * (Altitude_d[lev] - Altitude_d[lev - 1]))
...
__global__ void Calc_MOlength_Cdrag_BLdepth(double *pressure_d,
    ...
    if (id < num) {
        ...
        pt_surf_d[id]          = Tsurface_d[id] * pow(p_surf_d[id] / P_Ref, -kappa);
        Rho_int_d[id * (nv + 1)] =
        ...
        if (lev == 0) { // coefficient at surface
            ...
            if (Altitudeh_d[lev] <= f_surf_layer * bl_top_height_d[id]) { //inner layer
                ...
                if (RiB_d[id * (nvi) + 0] < 0) { //unstable
                    ...
                    double RiB_ratio = RiB_d[id * nvi + lev] / Ri_crit;
                    KM_d[id * (nvi) + lev] =
                        KVONKARMAN * Altitudeh_d[lev] * sqrt(CD_d[id]) * vh_lowest_d[id]
                        / (1 + RiB_ratio * log(Altitudeh_d[lev] / z_rough) / (1 - RiB_ratio));
                ...
                if (RiB_d[id * (nvi) + 0] < 0) { //unstable
                    ...
                    double RiB_ratio = RiB_d[id * nvi + sl_top_lev_d[id] + 1] / Ri_crit;
                    KM_d[id * (nvi) + lev] =
                        KVONKARMAN * f_surf_layer * bl_top_height_d[id] * sqrt(CD_d[id])
                        * vh_lowest_d[id] * z_scale
                        / (1
                           + RiB_ratio * log(Altitudeh_d[sl_top_lev_d[id] + 1] / z_rough)
                                 / (1 - RiB_ratio)); ///bl_top_lev_d is wrong!!
